...
def scrape_website(row):
    ...
    response_1 = requests.get(domain, headers=headers_1)
    if response_1.status_code != 200:
        response_2 = requests.get(domain,headers=headers_2)
        if response_2.status_code != 200:
            response_3 = requests.get(domain)
            if response_3.status_code!= 200:
                return index,fetch_and_extract_text(domain)
    ...
    if response_1.status_code == 200:
        ...
        inner_content = home_content + fetch_relevant_content(soup)
