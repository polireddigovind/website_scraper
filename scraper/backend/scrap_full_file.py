...
def fetch_and_extract_text(url: str) -> str:
    ...
    try:
        ...
        extracted_text = extract_text_from_html(response.content)
        extracted_text = extracted_text + fetch_relevant_content(soup,headers,url)
        return extracted_text
    except requests.exceptions.RequestException as e:
...
def fetch_home_content(domain,headers):
    ...
    # New code to limit text to 12000 characters
    if len(text) >12000:
        text = text[:12000]
...
def fetch_relevant_content(soup,headers,domain):
    ...
    for url in ex_urls:
        ...
        if check and len(check) > 0:
            ...
            fetch_data = fetch_home_content(url,headers)
            if len(fetch_data) > 0:
...
def scrape_website(row):
    ...
    for attempt in range(retries):
        try:
            ...
            inner_content = home_content + fetch_relevant_content(soup,headers,domain)
            ...
            print(f"error on {domain} is {e}")
            inner_content = fetch_and_extract_text_1(domain)
            return index , inner_content
        except Exception as e:
            print(f"error on second {domain} is {e}")
            inner_content = scrape_website_2(domain, zenrows_api_key)
            return index , inner_content
...
def process_rows(row):
    ...
    global scraped_record_count

    try:
        if records_processed >= 2000:
            print("Processed 2000 records, taking a 5-minute break...")
            row_count=f"Processed 2000 records, taking a 5-minute break... "
            if socketio:
        ...
        scraped_record_count +=1
        row_count=f"scraping url on row count {scraped_record_count} "
        if socketio:
            socketio.emit('value_update', {'value': row_count})
        ...
        if len(inner_content) > 20000:
            inner_content = inner_content[:20000]
...
temp=0
def model_process_rows(row):
    global temp  
    index=row["record_index"]
    row_count=f"model processing on row count {temp} "
    if socketio:
        socketio.emit('value_update', {'value': row_count})
    summary_data = summary_openai_prompt_1(row["data"],2)
    print(temp)
    if textPrompt_openai != '':
        summary_data = summary_Openai_model(summary_data,2)
    temp+=1
    df.at[index, 'Output_data'] = summary_data

def website_summary():
    global temp  # Ensure the global temp variable is accessed
    rows = []
    try:
        while True:
            response = supabase.table("full_data").select("record_index, title, data").limit(1000).execute()
            batch = response.data
            ...
            record_indices = [record['record_index'] for record in batch]
            supabase.table('full_data').delete().in_('record_index', record_indices).execute()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        executor.map(model_process_rows, rows)
