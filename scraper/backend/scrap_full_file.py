...
def fetch_and_extract_text(url: str) -> str:
    ...
    try:
        ...
        extracted_text = extract_text_from_html(response.content)
        extracted_text = extracted_text + fetch_relevant_content(soup,headers)
        return extracted_text
    except requests.exceptions.RequestException as e:
...
def fetch_relevant_content(soup,headers):
    ...
    if len(extracted_urls)>0 :
        batch = extracted_urls[0:min(len(extracted_urls),5)]
...
def scrape_website(row):
    ...
    for attempt in range(retries):
        try:
            ...
            inner_content = home_content + fetch_relevant_content(soup,headers)
